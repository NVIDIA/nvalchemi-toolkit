...
class DistillationJobSpec(BaseModel):
    ...
    offline, the reference dataset on-policy; ``strategy`` is the JSON-ready
    ...
    def _validate_on_policy_recipe(self) -> Self:
        ...
        if structures is not None and structures.recycle and settings.fmax is None:
            raise ValueError(
                ...
                "lifecycle ever backfills; got it set with fmax=None. Set fmax, "
                "or drop the flag."
            )
        self._validate_mixture(settings)
        devices = strategy_spec._devices_from_spec(self.strategy["devices"])
        if (
            settings.replay_device is not None
            and devices
            and not _same_device(torch.device(settings.replay_device), devices[0])
        ):
            raise ValueError(
                "the mixture is collated before training moves it, so the "
                "replay buffer and the reference dataset have to be staged on "
                "one device, and the CLI loads the reference dataset on the "
                "strategy's own; got "
                f"on_policy.replay_device={str(settings.replay_device)!r} against "
                f"strategy.devices[0]={str(devices[0])!r}. Leave replay_device "
                "unset to stage the frames where the reference dataset is loaded."
            )
        return self
    ...
    def _validate_mixture(self, settings: OnPolicySettings) -> None:
        ...
        settings : OnPolicySettings
            Scalar settings the segment-loop recipe sets, already validated.
        ...
        """
        if settings.replay_ratio == 1.0:
            raise ValueError(
                "replay_ratio=1 draws every sample of every batch from the "
                "replay buffer, and a recipe always names a reference dataset "
                "for dataset to open, so it would be policed for schema and "
                "device and then never sampled; lower replay_ratio to mix it in."
            )
...
def _restart_map_location(
    ...
    Notes
    -----
    A checkpoint records the device rank zero was pinned to, and loading every
    rank's weights onto it would stage the whole world's restore through one
    accelerator's memory, so the rank's own device is the default. The
    restored strategy's ``devices`` decide where the run continues either way.
    """
...
@click.option(
    "--dataset",
    required=True,
    help="Teacher-labeled store; the on-policy reference dataset.",
)
