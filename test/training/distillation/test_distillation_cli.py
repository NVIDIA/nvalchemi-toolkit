...
class TestRecipeScaffolds:
    ...
    def test_the_tiers_are_sizes_rather_than_architectures(
        ...
        """Every tier writes the same settings at a different width and depth."""
        widths = {}
...
class TestOnPolicyPreflight:
    ...
    def test_recycling_without_a_criterion_fails_at_report(
        ...
        assert result.exit_code != 0
        assert "InitialStructures.recycle" in _combined_output(result)
    ...
    def test_a_replay_only_recipe_fails_at_report(self, tmp_path: Path) -> None:
        """A recipe always names a reference dataset, so replay_ratio=1 can only be refused."""
        path = _write_on_policy_recipe(tmp_path)
        payload = json.loads(path.read_text())
...
class TestDistributedRecipeExecution:
    ...
    def test_a_map_location_naming_another_device_is_honoured(self) -> None:
        """A rank may stage its load elsewhere; the live strategy re-homes the state."""
        manager = _FakeManager(rank=1, device="cuda:1")

        assert distillation_cli._restart_map_location(manager, "cuda:0") == "cuda:0"
