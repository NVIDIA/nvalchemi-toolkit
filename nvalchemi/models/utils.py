...
"""Utilities for model wrappers, composition helpers, and unit conversion.

This module collects small shared helpers used across multiple model
wrappers.  It intentionally stays flat so common tensor normalization,
...
def collect_nondefault_repr_kwargs(
    *,
    ...
    """Collect constructor kwargs whose explicit values differ from defaults.

    Parameters
    ----------
    explicit_values
        Explicit constructor arguments supplied by the caller.
    defaults
        Default constructor values used for comparison.
    order
        Stable rendering order for the supported keyword arguments.

    Returns
    -------
    dict[str, object]
        Keyword arguments whose explicit values differ from their defaults.
    """
...
def initialize_model_repr(
    obj: object,
    *,
    static_kwargs: dict[str, object],
    kwarg_order: tuple[str, ...],
) -> None:
    """Attach repr metadata used by :func:`build_model_repr`.

    Parameters
    ----------
    obj
        Model or config object whose repr metadata should be initialized.
    static_kwargs
        Keyword arguments that should appear in the rendered repr.
    kwarg_order
        Stable rendering order for ``static_kwargs``.
    """

    setattr(obj, "_repr_kwargs", dict(static_kwargs))
    setattr(obj, "_repr_kwarg_order", kwarg_order)


def build_model_repr(obj: object) -> str:
    """Render a compact constructor-style repr for one model or config object.

    Parameters
    ----------
    obj
        Object previously initialized with :func:`initialize_model_repr`.

    Returns
    -------
    str
        Constructor-style representation containing only tracked kwargs.
    """
...
def replace_model_spec(
    ...
    *,
    ...
    neighbor_cutoff: float | None = _UNSET,
) -> ModelConfig:
    """Return a copy of one :class:`ModelConfig` with selected fields replaced."""

    requirement = spec.neighbor_config
    if neighbor_cutoff is not _UNSET:
        requirement = requirement.model_copy(update={"cutoff": neighbor_cutoff})
    return ModelConfig(
        required_inputs=spec.required_inputs,
        optional_inputs=spec.optional_inputs,
        outputs=spec.outputs,
        optional_outputs=dict(spec.optional_outputs),
        additive_outputs=spec.additive_outputs,
        use_autograd=spec.use_autograd,
        autograd_inputs=spec.autograd_inputs,
        autograd_outputs=spec.autograd_outputs,
        pbc_mode=spec.pbc_mode if pbc_mode is _UNSET else pbc_mode,
        neighbor_config=requirement,
    )


def normalize_batch_indices(
    positions: torch.Tensor,
    batch_idx: torch.Tensor | None,
) -> tuple[torch.Tensor, int]:
    """Return normalized batch indices and the number of systems."""
    ...
    if batch_idx is None:
        ...
        return batch_idx, 1
    normalized = batch_idx.long()
    num_graphs = int(normalized.max().item()) + 1 if normalized.numel() else 1
    return normalized, num_graphs


def mapping_get(data: Any, key: str, default: Any = None) -> Any:
    """Return one optional value from a dict-like model input mapping.

    Parameters
    ----------
    data
        Input mapping or batch-like object supporting ``__contains__`` and
        ``__getitem__``.
    key
        Input key to retrieve.
    default
...
def unpack_kernel_result(
    ...
    *,
    ...
    has_forces
        Whether the result is expected to include forces.
    has_virial
        Whether the result is expected to include a virial tensor.

    Returns
    -------
    tuple[torch.Tensor, torch.Tensor | None, torch.Tensor | None]
        Energy, forces, and virial tensors in normalized order.
    """
...
def normalize_node_charges(charges: torch.Tensor) -> torch.Tensor:
    """Return a one-dimensional node-charge tensor."""

    if charges.ndim > 1:
        return charges.squeeze(-1)
    return charges


def normalize_batched_cell(cell: torch.Tensor) -> torch.Tensor:
    """Ensure cell tensors have shape ``(B, 3, 3)``."""

    return cell.unsqueeze(0) if cell.ndim == 2 else cell


def infer_periodic_mode(
    *,
    ...
    """Infer whether one call is periodic and validate required periodic inputs."""

    if cell is None or pbc is None:
        if allow_non_pbc:
            return False
        raise ValueError(f"{model_name} requires both 'cell' and 'pbc'.")
    periodic = bool(torch.as_tensor(pbc).any().item())
    if not periodic and not allow_non_pbc:
        raise ValueError(f"{model_name} requires periodic boundary conditions.")
    return periodic


def resolve_matrix_neighbor_shifts(
    neighbor_matrix: torch.Tensor,
    neighbor_shifts: torch.Tensor | None,
    *,
    periodic: bool,
    model_name: str,
    allow_missing_non_pbc: bool,
) -> torch.Tensor:
    """Return matrix-format neighbor shifts, filling zeros when allowed."""

    if neighbor_shifts is not None:
        return neighbor_shifts.contiguous()
    if periodic and not allow_missing_non_pbc:
        raise ValueError(f"{model_name} requires 'neighbor_shifts' for periodic runs.")
    return torch.zeros(
...
def download_cached_file(
    *,
    ...
    """Download *url* into *cache_dir* and reuse the cached file when present."""
...
def aggregate_per_system_energy(
    ...
    # Cast back to original dtype after accumulation.
