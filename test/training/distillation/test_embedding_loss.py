...
class TestEmbeddingProjector:
    ...
    def test_projected_embeddings_stay_node_level_through_a_reassignment(
        self, small_batch: Batch
    ) -> None:
        """The public write registers the field at node level, where a later plain assignment keeps it."""
        student = _build_direct_force_teacher(hidden_dim=4, seed=1)
        student.compute_embeddings(small_batch)
        EmbeddingProjector(4, 6).compute_embeddings(small_batch)

        small_batch.node_embeddings = torch.zeros(small_batch.num_nodes, 6)

        assert "node_embeddings" in small_batch.keys["node"]
        assert small_batch["node_embeddings"].shape == (small_batch.num_nodes, 6)

    def test_compute_embeddings_projects_atomic_data_in_place(self) -> None:
        """A single system takes the plain-assignment path to the same result."""
        data = _build_atomic_data(n_atoms=3)
        _build_demo_model().compute_embeddings(data)
        EmbeddingProjector(data.node_embeddings.shape[-1], 6).compute_embeddings(data)
        assert data.node_embeddings.shape == (3, 6)
