...
"""Tests for :mod:`nvalchemi.training.distillation.losses.atomic_energy`.
...
_PREDICTION_KEYS = {
    ...
    "teacher_atomic_energies": "predicted_atomic_energies",
}
...
class TestAtomicEnergyMatchingLossValues:
    ...
    def test_global_mean_weights_every_atom_equally(self) -> None:
        ...
        loss_fn = AtomicEnergyMatchingLoss(normalize_by_atom_count=False)
        pred = torch.tensor([1.0, 2.0, 3.0, 4.0])
        loss = loss_fn(pred, torch.zeros(4), batch_idx=_UNEVEN_BATCH_IDX, num_graphs=2)
...
class TestAtomicEnergyMatchingLossMasking:
    ...
    def test_disabled_masking_propagates_nonfinite_targets(self) -> None:
        ...
        loss_fn = AtomicEnergyMatchingLoss(ignore_nonfinite=False)
        target = torch.tensor([0.0, 0.0, float("nan"), 0.0])
        loss = loss_fn(torch.ones(4), target, batch_idx=_UNEVEN_BATCH_IDX, num_graphs=2)
        assert torch.isnan(loss)


class TestAtomicEnergyMatchingLossPrecision:
...
class TestAtomicEnergyMatchingLossContract:
    ...
    def test_loss_declares_it_needs_no_evaluation_gradients(self) -> None:
        ...
        assert AtomicEnergyMatchingLoss().requires_eval_grad is False

    def test_default_keys_match_the_teacher_and_student_fields(self) -> None:
        """Defaults line up the teacher's atomic energies with the student's head."""
        loss_fn = AtomicEnergyMatchingLoss()
        assert loss_fn.target_key == "teacher_atomic_energies"
    ...
    def test_spec_round_trip_rebuilds_an_equivalent_loss(self) -> None:
        ...
        spec = loss_component_to_spec(
            AtomicEnergyMatchingLoss(
                target_key="teacher_atomic_energies",
        ...
        assert isinstance(rebuilt, AtomicEnergyMatchingLoss)
        assert rebuilt.normalize_by_atom_count is False
        assert rebuilt.ignore_nonfinite is False
        assert rebuilt.target_key == "teacher_atomic_energies"


class TestAtomicEnergyMatchingLossComposition:
    ...
    def test_composition_sums_weighted_teacher_terms(
        ...
        signals = ["energy", "forces", "atomic_energies"]
        ...
        predictions = {
            ...
            "predicted_atomic_energies": torch.zeros_like(
                batch.teacher_atomic_energies
            ),
        }
        ...
        assert out["per_component_weight"]["AtomicEnergyMatchingLoss"] == pytest.approx(
            0.5
        )
    ...
    def test_weight_schedule_ramps_the_teacher_term(
        ...
        batch = _make_labeled_batch(
            small_batch, direct_force_teacher, ["energy", "atomic_energies"]
        )
        loss_fn = ComposedLossFunction(
            [EnergyMSELoss(target_key="teacher_energy"), AtomicEnergyMatchingLoss()],
        ...
        assert (
            late["per_component_weight"]["AtomicEnergyMatchingLoss"]
            > early["per_component_weight"]["AtomicEnergyMatchingLoss"]
        )
...
class TestTeacherTargetLosses:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                AtomicEnergyMatchingLoss(), ["atomic_energies"], id="atomic_energies"
            ),
        ],
    )
