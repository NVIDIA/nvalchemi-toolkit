...
``replay_ratio`` is the setting the run turns on. It is the fraction of every
training batch drawn from generated frames, so ``1.0`` trains on generated data
alone while a lower value keeps part of each batch on a fixed reference
dataset. That dataset has to be teacher-labeled and carry the same fields a
...
# -----------------------------
# The run is three segments long: each one propagates ``GENERATION_STEPS``
# steps and then spends ``TRAINING_STEPS_PER_SEGMENT`` optimizer steps on the
# mixture, until ``NUM_STEPS`` is reached. Scaling this to a real workflow
...
# initial batch and any later backfill both draw from. Left unbudgeted, as
# here, it propagates every row it owns as one batch, so ``NUM_INITIAL`` *is*
...
# because both mixture sources are collated into one batch before the strategy
# moves it.
...
# rather than regenerating them. Its schema is the contract the reference
# dataset had to meet: the structure, whatever travels with it, and the
