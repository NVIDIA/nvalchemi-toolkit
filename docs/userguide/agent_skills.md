...
| `nvalchemi-training-api` | How to configure training workflows with {py:class}`~nvalchemi.training.TrainingStrategy`: losses, optimizers, schedulers, validation, hooks, and restartable checkpoints. | {ref}`training_guide` |
...
| `nvalchemi-distributed-training` | How to scale training across GPUs and nodes with `DistributedManager`, `DDPHook`, distributed samplers, and rank-safe validation and checkpointing. | {ref}`distributed_manager_guide` |
