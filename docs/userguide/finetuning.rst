...
:class:`~nvalchemi.training.TrainingStrategy`. The
:class:`~nvalchemi.training.FineTuningStrategy` convenience wrapper adds two
pieces that are common in transfer-learning workflows:

* patch one or more :class:`torch.nn.Module` children before optimizers are
  built;
...
* a dataset or dataloader that yields :class:`~nvalchemi.data.Batch` objects;
...
existing :class:`torch.nn.Module` or add a new child module, but the model's
...
captured. Use :func:`~nvalchemi.training.create_model_spec` for declarative
...
:class:`~nvalchemi.training.hooks.TrainableParameterHook`.
