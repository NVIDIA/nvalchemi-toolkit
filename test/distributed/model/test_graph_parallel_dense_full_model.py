...
"""End-to-end gate for the ``gp_replicate_geometry`` graph-parallel path.

A single-pass pairwise dense-``neighbor_matrix`` MLIP
(:class:`ToyGraphParallelDenseFullWrapper`, PME's fused-kernel shape) run
single-process over all atoms and graph-parallel through ``DistributedModel``,
which must agree on energy + owned forces.

Exercises the full-geometry dense path: ``_graph_parallel_dense_full_autograd``
(replicated geometry, ``neighbor_matrix`` masked to owned receivers via
``num_neighbors``), the owned-aware ``node_energy_key`` reduction, and framework
autograd of the owned energy over the full-position leaf with cross-rank SUM +
owned slice. This is the machinery PME real-space rides (#148).
"""
...
def _worker(rank: int, world: int) -> None:
    ...
    if rank == 0:
        print(f"[gp-dense-full w={world}] energy + owned forces match single-process")
    dist.barrier()
    dist.destroy_process_group()


def test_graph_parallel_dense_full_model_2ranks() -> None:
    mp.spawn(_worker, args=(2,), nprocs=2)


def test_graph_parallel_dense_full_model_3ranks() -> None:
