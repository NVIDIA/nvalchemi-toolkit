...
"""Multi-GPU regression: PME under node-partition graph-parallel.

The graph-parallel sibling of ``test_pme_multigpu`` (halo). A 2-rank
``DistributedModel(PMEModelWrapper)`` on the ``GRAPH_PARTITION`` strategy must
match the single-GPU reference on total energy and per-atom forces.

Exercises the correctness-first PME GP path: PME's ``_distribution_spec_gp``
...
geometry replicated per rank (correct reciprocal from the full charge set), the
dense ``neighbor_matrix`` masked to owned receivers (partitioned real-space), the
owned-aware ``node_energy_key`` sum, and framework autograd of the owned energy
over the full-position leaf. Requires ``hybrid_forces=False``.

Run (box, 2 GPUs, ``NCCL_P2P_DISABLE=1`` on trx40-03)::

    NVALCHEMI_PME_N_SIDE=3 NVALCHEMI_PME_BOX=8.46 \\
        pytest test/distributed/model/test_pme_gp_multigpu.py -v -s
"""
