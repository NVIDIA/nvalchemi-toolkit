...
PET_MODEL_PATH = os.environ.get("PET_MODEL_PATH", "")
USE_PET = False
...
if PET_MODEL_PATH:
    try:
        ...
        model = PETWrapper.from_checkpoint(
            checkpoint_path=PET_MODEL_PATH,
            device=torch.device("cuda" if torch.cuda.is_available() else "cpu"),
        )
        print(f"Using PET model from {PET_MODEL_PATH}")
        USE_PET = True
    except Exception as exc:
        print(f"Failed to load PET ({exc}). Falling back to LJ.")
...
if not USE_PET:
    ...
    print("Using LJ model (set PET_MODEL_PATH=/path/to/model.ckpt to use PET)")
...
print(f"  Model used    : {'PET' if USE_PET else 'Lennard-Jones (fallback)'}")
