...
class NEB(DynamicsStrategy):
    ...
    Attributes
    ----------
    convergence_hook : ConvergenceHook or None
        Optional override for the final or only optimizer stage. When provided,
        it takes precedence over ``fmax``. Otherwise, :meth:`build_engine`
        constructs the force criterion from ``fmax``. Overrides must use the
        standard hook type with ``frequency=1``.
    regular_convergence_hook : ConvergenceHook or None
        Optional override for the preliminary stage of an ``after_regular`` run.
        When provided, it takes precedence over ``climbing.regular_fmax`` and
        ``fmax``. Otherwise, :meth:`build_engine` constructs the force
        criterion from those thresholds. It has the same type and frequency
        constraints and is otherwise ignored.
    """
    ...
    fmax: float = Field(
        ...
        gt=0,
        description=(
            "Force threshold for convergence of the final or only stage; ignored "
            "when convergence_hook is provided."
        ),
    )
    ...
    convergence_hook: ConvergenceHook | None = Field(
        default=None,
        exclude=True,
        description="Final-stage convergence override; takes precedence over fmax.",
    )
    regular_convergence_hook: ConvergenceHook | None = Field(
        default=None,
        exclude=True,
        description=(
            "Preliminary-stage convergence override; takes precedence over "
            "climbing.regular_fmax and fmax."
        ),
    )
    ...
    def _build_convergence_hook(
        ...
        *,
        ...
        template = (
            self.regular_convergence_hook if regular_stage else self.convergence_hook
        )
        if template is None:
