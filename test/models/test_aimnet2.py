...
def real_aimnet2_derivative_wrapper():
    """Load the CUDA AIMNet2 checkpoint for derivative qualification."""
    if not torch.cuda.is_available():
        pytest.skip("CUDA required for the AIMNet2 derivative qualification")
...
class TestAIMNet2Derivatives:
    """Mock preflight and real CUDA qualification for AIMNet2 derivatives."""

    def test_mock_hvp_is_energy_only(self, mock_model, simple_batch):
        wrapper = _make_wrapper(mock_model)
        result = wrapper.hessian_vector_product(
            simple_batch, torch.randn_like(simple_batch.positions)
        )

        assert result.shape == simple_batch.positions.shape
        assert not result.requires_grad

    def test_mock_dense_loop_and_vmap_agree(self, mock_model, simple_batch):
        wrapper = _make_wrapper(mock_model)
        loop_batch = simple_batch.clone()
        vmap_batch = simple_batch.clone()
        wrapper.compute_hessian(loop_batch, strategy="loop", row_chunk_size=2)
        wrapper.compute_hessian(vmap_batch, strategy="vmap", row_chunk_size=2)
